import { CalendarDays, CheckCircle2, ChevronRight, CloudOff, Dumbbell, Pencil, Play, RefreshCw, Trophy } from 'lucide-react';
...
function calculatePersonalRecords(sessionRows: any[]): PersonalRecord[] {
  const records = new Map<number, PersonalRecord>();
  for (const session of sessionRows) for (const workoutExercise of session.workout_exercises ?? []) {
...
export function WorkoutsPage() {
  const supabase = getSupabase(); const navigate = useNavigate(); const { user, profile } = useAuth();
  const [routines, setRoutines] = useState<Routine[]>([]); const [sessions, setSessions] = useState<any[]>([]); const [records, setRecords] = useState<PersonalRecord[]>([]);
  const [loading, setLoading] = useState(true); const [starting, setStarting] = useState<string | null>(null); const [error, setError] = useState('');

  const load = async () => {
    if (!user) return; setLoading(true); setError('');
    const [cachedRoutines, cachedSessions, cachedRecords] = await Promise.all([getCached<Routine[]>(cacheKeys.routines(user.id)), getCached<any[]>(cacheKeys.sessions(user.id)), getCached<PersonalRecord[]>(`personal-records:${user.id}`)]);
    if (cachedRoutines) setRoutines(cachedRoutines); if (cachedSessions) setSessions(cachedSessions); if (cachedRecords) setRecords(cachedRecords); setLoading(false);
    if (!navigator.onLine) { if (!cachedRoutines?.length) setError('Conéctate una vez para descargar tus rutinas a este dispositivo.'); return; }
    const [routineResult, sessionResult, recordResult] = await Promise.all([
      ...
      supabase.from('workout_sessions').select('id,routine_id,session_date,finished_at,started_at').eq('user_id', user.id).order('session_date', { ascending: false }).limit(40),
    ...
    if (routineResult.error) { if (!cachedRoutines?.length) setError(routineResult.error.message); return; }
    ...
    setRoutines(sorted); setSessions(sessionResult.data ?? []); setRecords(nextRecords);
    await Promise.all([setCached(cacheKeys.routines(user.id), sorted), setCached(cacheKeys.sessions(user.id), sessionResult.data ?? []), setCached(`personal-records:${user.id}`, nextRecords)]);
  };
  useEffect(() => { void load(); }, [user]);

  const startRoutine = async (routine: Routine) => {
    if (!user || !profile) return; setStarting(routine.id); setError('');
    const sessionId = crypto.randomUUID(), sessionDate = dateInTimezone(profile.timezone), startedAt = new Date().toISOString();
    const workoutExercises = routine.routine_exercises.map((item) => ({ id: crypto.randomUUID(), session_id: sessionId, planned_routine_exercise_id: item.id, exercise_id: item.exercise.id, position: item.position, exercise: item.exercise, planned: { target_sets: item.target_sets, rep_min: item.rep_min, rep_max: item.rep_max, rir_target: item.rir_target }, workout_sets: Array.from({ length: item.target_sets }, (_, index) => ({ id: crypto.randomUUID(), set_number: index + 1, weight_kg: null, reps: null, rir: null, completed: false })) }));
    ...
    await Promise.all([setCached(cacheKeys.workoutSession(sessionId), localSession), cacheSessionSummary(user.id, { id: sessionId, routine_id: routine.id, session_date: sessionDate, started_at: startedAt, finished_at: null })]);
  ...
  return <div className="page-grid">
    <section className="page-heading simple"><div><p className="eyebrow">ENTRENAMIENTO</p><h1>Tu programa · {routines.length} día{routines.length === 1 ? '' : 's'}</h1><p className="muted">PPL, Full Body, Torso/Pierna o cualquier combinación.</p></div><div className="button-row"><button className="secondary-button" onClick={load}><RefreshCw size={17} /> Actualizar</button><Link className="primary-button" to="/rutinas"><Pencil size={17} /> Editar programa</Link></div></section>
    {!navigator.onLine && <div className="alert success"><CloudOff size={16} /> Modo offline: los cambios se sincronizarán al volver internet.</div>}{error && <div className="alert error">{error}</div>}
    <section className="routine-grid">{routines.map((routine, index) => { const last = sessions.find((session) => session.routine_id === routine.id); return <article className="routine-card" key={routine.id}>
      <div className="routine-card-top"><div className="metric-icon"><Dumbbell /></div><div><span>Día {index + 1}</span><h2>{routine.name}</h2></div>{last?.finished_at ? <span className="status-chip green"><CheckCircle2 size={14} /> Hecha</span> : <span className="status-chip">Disponible</span>}</div>
      ...
      {!routine.routine_exercises.length && <div className="alert error">Agrega al menos un ejercicio desde “Editar programa”.</div>}{last && <p className="last-session"><CalendarDays size={15} /> Última: {last.session_date}</p>}
      <button className="primary-button" onClick={() => startRoutine(routine)} disabled={starting === routine.id || !routine.routine_exercises.length}><Play size={17} /> {starting === routine.id ? 'Preparando…' : 'Iniciar rutina'} <ChevronRight size={17} /></button>
    </article>; })}</section>
