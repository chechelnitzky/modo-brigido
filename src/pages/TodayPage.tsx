import { Activity, Check, ChevronRight, CloudOff, Dumbbell, Flame, Footprints, Moon, Ruler, Save, Scale, Zap } from 'lucide-react';
...
export function TodayPage() {
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      if (cancelled) return;
      if (loadError) { if (!cached) setError(loadError.message); return; }
  ...
  const bodyComposition = useMemo(() => profile && log ? estimateBodyCompositionForLog(profile, log) : null, [profile, log]);
  const update = <K extends keyof DailyLog>(key: K, value: DailyLog[K]) => { setSavedState('none'); setLog((current) => current ? { ...current, [key]: value } : current); };

  const save = async () => {
    if (!log || !user) return;
    setSaving(true); setError('');
    const next: DailyLog = { ...log, id: log.id ?? crypto.randomUUID(), user_id: user.id, log_date: today };
    setLog(next); await cacheDailyLog(next);
    const result = await saveMutation({ operation: 'upsert', table: 'daily_logs', payload: { ...next, updated_at: new Date().toISOString() }, onConflict: 'user_id,log_date', dedupeKey: `daily:${user.id}:${today}` });
    setSavedState(result === 'synced' ? 'synced' : 'offline'); setSaving(false);
  };

  if (!profile || !log) return <div className="page-loading">Cargando tu día…</div>;
  const calories = log.calories ?? 0, protein = log.protein_g ?? 0, steps = log.steps ?? 0;

  return (
    <div className="page-grid dashboard-page">
      <section className="page-heading"><div><p className="eyebrow">HOY</p><h1>{prettyDate(today)}</h1><p className="muted">Un día consistente vale más que un día perfecto.</p></div><ScoreRing value={score} /></section>
