...
export function NutritionPage() {
  ...
  const save = async (next: DailyLog) => {
    const local = { ...next, id: next.id ?? crypto.randomUUID(), user_id: user.id, log_date: selectedDate };
    setLog(local);
    setSaving(true);
    ...
    const result = await saveMutation({
      ...
      onConflict: 'user_id,log_date', dedupeKey: `daily:${user.id}:${selectedDate}`
    });
  ...
  return (
    <div className="page-grid">
      <section className="page-heading simple"><div><p className="eyebrow">{isToday ? 'NUTRICIÓN DE HOY' : 'NUTRICIÓN HISTÓRICA'}</p><h1>{prettyDate(selectedDate)}</h1><p className="muted">Alcanza calorías y proteína sin complicarte.</p></div></section>
      <section className="two-column">
        ...
        <article className="panel nutrition-card"><div className="section-title"><div className="metric-icon"><Zap /></div><div><span>Proteína de esta fecha</span><h2>{log.protein_g ?? 0} / {profile.protein_target} g</h2></div></div><ProgressBar value={log.protein_g ?? 0} max={profile.protein_target} /><p className="muted">Objetivo mínimo para apoyar el entrenamiento.</p><input type="number" value={log.protein_g ?? ''} onChange={(e) => setLog({ ...log, protein_g: numberOrNull(e.target.value) })} onBlur={() => save(log)} /></article>
      </section>
