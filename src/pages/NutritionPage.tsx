import { CheckCircle2, Flame, Plus, Salad, Zap } from 'lucide-react';
...
export function NutritionPage() {
  ...
  useEffect(() => {
    if (!user) return;
    supabase.from('daily_logs').select('*').eq('user_id', user.id).eq('log_date', today).maybeSingle().then(({ data }) => {
      setLog((data as DailyLog | null) ?? {
        user_id: user.id, log_date: today, weight_kg: null, waist_cm: null, sleep_score: null,
        energy_score: null, hunger_score: null, cannabis: null, calories: null, protein_g: null,
  ...
  const save = async (next: DailyLog) => {
    setLog(next);
    setSaving(true);
    const payload = { ...next, user_id: user.id, log_date: today, updated_at: new Date().toISOString() };
    delete payload.id;
